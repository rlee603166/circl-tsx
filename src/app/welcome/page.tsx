...
const WelcomeScreen = () => {
  ...
  return (
    <div className="max-h-[100vh] min-h-screen flex relative">
      ...
      <div className="w-full h-[100vh] lg:w-1/2 flex justify-center px-8 bg-gray-50">
      <div className="h-[615px] my-auto flex items-start">
              ...
              <div className="space-y-4 mb-3">
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full px-4 py-4 border-2 rounded-xl bg-white transition-all duration-200 focus:outline-none peer ${emailError
                        ? 'border-red-400 focus:border-red-400'
                        : emailFocused
                          ? 'border-[#21B8CD] focus:border-[#21B8CD]'
                          : 'border-gray-200 focus:border-[#21B8CD]'
                      }`}
                    placeholder=" "
                  />
                  <label
                    ...
                    className={`absolute left-4 transition-all duration-200 pointer-events-none ${emailFocused || email
                        ? '-top-2.5 text-sm bg-white px-2 ' + (emailError ? 'text-red-400' : 'text-[#21B8CD]')
                ...
                {isLogin && showPassword && (
                  <div className="animate-in slide-in-from-top-2 duration-300 relative">
                    <input
                      ...
                      className={`w-full px-4 py-4 border-2 rounded-xl bg-white transition-all duration-200 focus:outline-none peer ${passwordError
                          ? 'border-red-400 focus:border-red-400'
                          : passwordFocused
                            ? 'border-[#21B8CD] focus:border-[#21B8CD]'
                            : 'border-gray-200 focus:border-[#21B8CD]'
                    ...
                    <label
                      ...
                      className={`absolute left-4 transition-all duration-200 pointer-events-none ${passwordFocused || password
                          ? '-top-2.5 text-sm bg-white px-2 ' + (passwordError ? 'text-red-400' : 'text-[#21B8CD]')
                          : 'top-4 text-base text-gray-400'
                        }`}
                    >
                      Password
      ...
      <div className="w-full lg:w-1/2 flex justify-center px-8 bg-white">
        <div className="w-full max-w-2xl my-auto py-auto h-[600px] ">
                    ...
                    {showAnswer && answerText.length === queries[currentSlide].answer.length && (
                      <div className="mt-4 space-y-3">
                        {queries[currentSlide].type === 'profiles' ? (
                          ...
                          (queries[currentSlide].bullets as any[]).map((tenure, index) => (
                            <div key={index} className={`transition-all duration-500 ${showBullets[index] ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'}`}>
