  ...
  return (
    <section id="cta" className="py-20 px-6 md:px-10 lg:px-0">
      <div className="max-w-7xl mx-auto">
        <div className="max-w-6xl mx-auto rounded-3xl bg-purple-50 p-8 md:p-12 lg:p-20 text-center">
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-light text-gray-900 mb-6 md:mb-8">Be the First to Know</h2>
          <p className="text-lg md:text-xl lg:text-2xl text-gray-600 font-light max-w-2xl mx-auto mb-8 md:mb-12">
          ...
          <div className="flex flex-col sm:flex-row gap-4 md:gap-6 justify-center">
            ...
            <a href="#features" className="px-6 md:px-8 py-3 md:py-4 rounded-full bg-white hover:bg-gray-50 text-gray-700 font-light text-base md:text-lg border border-gray-200 transition-all hover:scale-105 focus:scale-105">
              Learn About Features
            </a>
          </div>
          
          <div className="mt-12 md:mt-20 max-w-5xl mx-auto">
            <div className="backdrop-blur-xl bg-white/60 rounded-3xl border border-white/20 shadow-2xl p-4 md:p-6 lg:p-8 relative overflow-hidden">
                ...
                {!activeQuery ? (
                  <div className="space-y-4 md:space-y-6">
                    <div className="flex items-center space-x-3 md:space-x-4 w-full">
                      ...
                      <div className="flex-1 bg-white/60 rounded-xl p-3 md:p-4 text-left text-slate-700 font-normal text-base md:text-lg min-h-12 md:min-h-14 flex items-center">
                        <span className="text-slate-400">Ask about career paths...</span>
                      </div>
                    </div>
                    
                    <div className="text-left">
                      <p className="text-slate-600 text-sm md:text-base mb-3 md:mb-4 font-light">Try these examples:</p>
                      <div className="space-y-2 md:space-y-3">
                        <button
                          onClick={() => handleQueryClick('consulting')}
                          className="w-full text-left text-slate-600 text-sm md:text-base p-3 md:p-4 rounded-xl bg-white/80 hover:bg-white transition-all cursor-pointer border border-white/40 hover:shadow-md group"
                  ...
                  <div className="space-y-4 md:space-y-6">
                    <div className="flex items-center space-x-3 md:space-x-4 w-full">
                      ...
                      <div className="flex-1 bg-white/60 rounded-xl p-3 md:p-4 text-left text-slate-700 font-normal text-base md:text-lg min-h-12 md:min-h-14">
                        ...
                        <div className="mt-3 md:mt-4 space-y-2 md:space-y-3">
                          {activeQuery === 'consulting' ? (
                            ...
                            (queries.skills.bullets as Array<{skill: string; demand: string; growth: string}>).map((skill, index) => (
                              <div key={index} className={`transition-all duration-500 ${showBullets[index] ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'}`}>
                                <div className="bg-gradient-to-r from-orange-50 to-amber-50 rounded-xl p-3 md:p-4 border border-orange-200 w-full">
                                  <div className="flex items-center justify-between w-full">
                                    <div className="flex-1 pr-2">
                                      <h4 className="font-medium text-slate-800 text-sm md:text-base mb-1">{skill.skill}</h4>
                                      <div className="flex items-center space-x-2">
                                        <span className="text-orange-700 text-base md:text-lg font-semibold">{skill.demand}</span>
                            ...
                            (queries.remote.bullets as Array<{location: string; percentage: string; trend: string}>).map((geo, index) => (
                              <div key={index} className={`transition-all duration-500 ${showBullets[index] ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'}`}>
                                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-3 md:p-4 border border-blue-200 w-full">
                                  <div className="flex items-center justify-between w-full">
                                    <div className="flex-1 pr-2">
                                      <h4 className="font-medium text-slate-800 text-sm md:text-base mb-1">{geo.location}</h4>
                                      <div className="flex items-center space-x-2">
